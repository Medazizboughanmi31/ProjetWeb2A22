<?php
	...
	class reclamationC {
		function afficherreclamations($etat){
			...
			function afficherreclamationsnseen($etat){
				$sql="SELECT * FROM reclamation where etat=$etat and seen =0";
				$db = config::getConnexion();
				try{
					$liste = $db->query($sql);
					return $liste;
		...
		function compterreclamations(){
			$sql="SELECT count(*) as num,etat FROM reclamation GROUP BY ETAT ORDER BY ETAT ";
			$db = config::getConnexion();
			try{
				$liste = $db->query($sql);
				return $liste;
		...
		function modifierreclamation($reclamation, $id_rec){
			...
			function modifierreclamationnseen($id_rec){
			try {
				$db = config::getConnexion();
				$query = $db->prepare(
					'UPDATE reclamation SET 
